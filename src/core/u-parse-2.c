...
#define IN_REVERSE(p) (p->flags & PF_REVERSE)
...
#define ADVANCE(p, i) (IN_REVERSE(p) ? --i : ++i)
#define NEXT(p, i) (IN_REVERSE(p) ? i-1 : i+1)
...
***********************************************************************/
{
	...
	REBSER *ser;	// temp series
	REBCNT i;		// temp index
	REBCNT flags = parse->flags | AM_FIND_MATCH | AM_FIND_TAIL; 

	if IN_REVERSE(parse) flags = flags ^ AM_FIND_TAIL;   // return head of match (not tail) when in REVERSE
	...
	case REB_CHAR: 				// !!! Needs update for REVERSE
		i = IN_REVERSE(parse)? index-1 : index;
		if (HAS_CASE(parse))
			index = (VAL_CHAR(item) == GET_ANY_CHAR(series, i)) ? ADVANCE(parse, index) : NOT_FOUND;
		else
			index = (UP_CASE(VAL_CHAR(item)) == UP_CASE(GET_ANY_CHAR(series, i))) ? ADVANCE(parse, index) : NOT_FOUND;
		break;

	case REB_EMAIL:
	case REB_STRING:
	case REB_BINARY: 
		index = Find_Str_Str(series, 0, IN_REVERSE(parse)? (index - VAL_LEN(item)) : index, SERIES_TAIL(series), 1, VAL_SERIES(item), VAL_INDEX(item), VAL_LEN(item), flags);
		break;

	// Do we match to a char set?
	case REB_BITSET:			// !!! Needs update for REVERSE
		i = IN_REVERSE(parse)? index-1 : index;
		flags = Check_Bit(VAL_SERIES(item), GET_ANY_CHAR(series, i), !HAS_CASE(parse));
		index = flags ? ADVANCE(parse, index) : NOT_FOUND;
		break;
/*
...
//	case REB_ISSUE:
		...
		// !! Not sure about this.  The tag datatype needs to be 'formed' with angle brackets prior to matching -- Dan Lee
		ser = Copy_Form_Value(item, 0);
		index = Find_Str_Str(series, 0, IN_REVERSE(parse)? (index - ser->tail) : index, SERIES_TAIL(series), 1, ser, 0, ser->tail, flags);
		break;
...
{
	// !!! THIS CODE NEEDS CLEANUP AND REWRITE BASED ON OTHER CHANGES
	REBCNT i = IN_REVERSE(parse)? index-1 : index;
	REBSER *series = parse->series;
	REBVAL *blk = BLK_SKIP(series, i);
	...
	// Parse a sub-rule block:
	case REB_BLOCK:
		index = Parse_Rules_Loop(parse, index, VAL_BLK_DATA(item), depth); // !!! update for REVERSE
		break;
...
***********************************************************************/
{
	// !!! TODO: Make TO/THRU work in REVERSE for block series
	REBSER *series = parse->series;
	...
	for (; index <= series->tail & index >= 0; ADVANCE(parse, index)) {
...
***********************************************************************/
{
	...
	// END
		...
		i = To_Thru(parse, index, item, is_thru);  // !!! Does this also work for REVERSE?
	}
	else {
		if (IS_BLOCK_INPUT(parse)) {
			...
			i = Find_Block(series, index, series->tail, item, 1, parse->flags | AM_FIND_CASE | (IN_REVERSE(parse)? AM_FIND_REVERSE : 0), 1);	// !!! Condense
			...
			if (ANY_BINSTR(item)) {
				if (!IS_STRING(item) && !IS_BINARY(item)) {
					...
					ser = Copy_Form_Value(item, 0);
					i = Find_Str_Str(series, 0, index, series->tail, IN_REVERSE(parse)? -1 : 1, ser, 0, ser->tail, HAS_CASE(parse));
					if (i != NOT_FOUND && is_thru) i += ser->tail;
				}
				else {
					i = Find_Str_Str(series, 0, index, series->tail, IN_REVERSE(parse)? -1 : 1, VAL_SERIES(item), VAL_INDEX(item), VAL_LEN(item), HAS_CASE(parse));
			...
			if (VAL_TYPE(item) <= REB_UNSET || VAL_TYPE(item) >= REB_MAP) Trap1(RE_PARSE_RULE, item);
		}
	}
	...
	while (NOT_END(rules)) {
		...
		for (count = 0; count < maxcount;) {
			...
			if (IS_WORD(item)) {
				...
				case SYM_SKIP:
					i = (index < series->tail) ? NEXT(parse, index) : NOT_FOUND;
					break;
				...
				case SYM_THRU:
					...
					item = Get_Parse_Value(rules);
					rulen = 1;
					i = Parse_To(parse, index, item, IN_REVERSE(parse)? (cmd != SYM_THRU) : (cmd == SYM_THRU));	// Reverse mode inverts the meaning of TO and THRU
					break;

				case SYM_REVERSE:
					parse->flags = (parse->flags ^ PF_REVERSE);
					break;
...
post:
		...
		if (flags) {
			...
			if (index == NOT_FOUND) { // Failure actions:
				...
				count = (begin > index) // how much we advanced the input
					  ? begin - index 
					  : index - begin; 
				i = (begin > index) ? index : begin; // start at the smallest index
				if (GET_FLAG(flags, PF_COPY)) {
					ser = (IS_BLOCK_INPUT(parse))
						? Copy_Block_Len(series, i, count)
						: Copy_String(series, i, count); // condenses
					Set_Var_Series(word, parse->type, ser, 0);
				}
				else if (GET_FLAG(flags, PF_SET_OR_COPY)) {		// !!! may need update for REVERSE
					if (IS_BLOCK_INPUT(parse)) {
						...
						item = Get_Var_Safe(word);		// !!! may need update for REVERSE
						if (count == 0) SET_NONE(item);
						else {
				...
				if (GET_FLAG(flags, PF_RETURN)) {		// !!! may need update for REVERSE
					ser = (IS_BLOCK_INPUT(parse))
						? Copy_Block_Len(series, i, count)
						: Copy_String(series, i, count); // condenses
				...
				if (GET_FLAG(flags, PF_REMOVE)) {
					if (count) Remove_Series(series, i, count);
					index = i;
				}
				if (flags & (1<<PF_INSERT | 1<<PF_CHANGE)) {		// !!! may need update for REVERSE
					...
					// !!! What is this? Will REVERSE affect it?
					// CHECK FOR QUOTE!!
				...
				if (GET_FLAG(flags, PF_AND)) index = i;
			}
